# =============================================================================
# E-D model (baseline covariates + clinical indicators + 70 pollutants,
# no biological feature matrix)
...
# Feature groups (no biological feature matrix is used in this model)
groups <- list(
  Baseline = baseline_vars,
  Clinical = clinical_vars,
  Exposome = pollutants_data_cols
)
...
res_ED <- run_sg_model(df_sg, voca, "ED_Model_NoPathway")
...
for (ri in seq_len(nrow(rob_grid))) {
  ...
  design_fixed <- benchmark_grid(tasks = res_ED$task_final, learners = learner_fixed,
                                 resamplings = res_ED$rsmp_final)
...
for (g in names(groups)) {
  ...
  base_imp_ED[[g]] <- imp_df_temp
}

# -----------------------------------------------------------------------------
# Feature importance plot
# -----------------------------------------------------------------------------
imp_ED <- res_ED$imp_df %>%
...
p_imp_ED <- ggplot(imp_ED, aes(x = Importance_pct, y = Features, fill = GroupDetail)) +
  geom_col() +
  scale_fill_manual(values = color_palette_ED, name = "Variable Type") +
  labs(x = "Relative Importance", y = NULL,
       title = "Feature Importance (E-D Model, No Biological Pathways)") +
  theme_minimal() +
  theme(legend.position = "bottom")

ggsave("importance_ed.png", p_imp_ED,
       width = 8, height = max(5, nrow(imp_ED) * 0.3))
