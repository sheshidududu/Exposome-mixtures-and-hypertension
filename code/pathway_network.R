# =============================================================================
# E-Pa-D network: exposure-pathway-disease network in Cytoscape
# =============================================================================
...
Nodes_Pa <- bind_rows(nodes_source, nodes_target) %>%
  distinct(id, .keep_all = TRUE) %>%
  mutate(
...
print(paste("NA values in edge table:", sum(is.na(Edges_Pa))))

# -----------------------------------------------------------------------------
# Node degree and size
# -----------------------------------------------------------------------------
degree_source <- Edges_Pa %>% count(source, name = "n_source")
degree_target <- Edges_Pa %>% count(target, name = "n_target")
...
print(paste("Nodes:", nrow(Nodes_Pa)))
print(paste("Edges:", nrow(Edges_Pa)))
print("Node groups:")
print(table(Nodes_Pa$group))
...
createNetworkFromDataFrames(
  nodes = Nodes_Pa,
  edges = Edges_Pa,
  title = "Exposure-Pathway-Hypertension Network",
...
setNodeColorMapping(
  mapping.type = 'd',
  table.column = 'group',
  table.column.values = c("Exposure", "Disease", "Pathway"),
  colors = c("#E74C3C", "#3498DB", "#66CDAA"),
