# =============================================================================
# E-Pa-D model (baseline covariates + clinical indicators + 70 pollutants
# + Pathway biological feature matrix)
# Stacked generalisation pipeline with elastic-net pathway feature screening
# =============================================================================
...
#   pathway_matrix_raw: pollutant-pathway matrix from the ExposomeX platform
...
# pathway_matrix_raw <- ...   # load Pathway matrix here
...
# -----------------------------------------------------------------------------
# Feature engineering: Pathway feature activity (exposure matrix %*% Pathway matrix)
# -----------------------------------------------------------------------------
...
available_exids <- intersect(existing_exids, rownames(pathway_matrix_raw))

pathway_features <- character(0)
if (length(available_exids) > 0) {
  ...
  B <- pathway_matrix_raw[available_exids, , drop = FALSE] %>% as.matrix()
  B[is.na(B)] <- 0
  T_mat <- P %*% B
  colnames(T_mat) <- colnames(B)
  T_scaled <- apply(T_mat, 2, scale_to_01) %>%
    as_tibble() %>%
    rename_with(~ paste0("Pathway_", .x))
  eSet$Data <- bind_cols(eSet$Data, T_scaled)
  pathway_features <- colnames(T_scaled)
} else {
  warning("No pollutants could be matched to the Pathway matrix; Pathway features skipped.")
}

# -----------------------------------------------------------------------------
# Elastic-net screening of Pathway features
# (alpha = 0.5, 5-fold CV, lambda.min scaled by lambda_factor)
# -----------------------------------------------------------------------------
select_features_elasticnet <- function(features, data, target,
                                       ...
                                       lambda_factor = 0.1,
                                       seed = 20240821) {
  cat("\n===== Elastic-net screening (alpha=0.5, 5-fold CV, lambda.min, factor=",
      lambda_factor, ") =====\n")
  df <- data %>% select(all_of(c(target, features))) %>% drop_na()
  if (ncol(df) < 2 || nrow(df) == 0) {
    warning("Insufficient data; no Pathway features returned.")
    return(character(0))
  }
  ...
  if (length(selected) == 0) {
    warning("No Pathway feature selected at the current lambda_factor.")
  }
  return(selected)
}

if (length(pathway_features) > 0) {
  pathway_features <- select_features_elasticnet(
  ...
  cat("Pathway feature count after screening:", length(pathway_features), "\n")
} else {
  cat("No Pathway features; elastic-net screening skipped.\n")
}
...
groups <- list(
  ...
  Exposome = c(existing_pollutant_cols, pathway_features)
)
...
imp_sg <- imp_sg %>%
  mutate(GroupDetail = case_when(
    ...
    VarType == "Exposome" & str_detect(Features, "^Exposome_Pathway_") ~ "GO",
    VarType == "Exposome" & !str_detect(Features, "^Exposome_Pathway_") ~ "Pollutant",
    TRUE ~ "Other"
  ))
...
p_imp <- ggplot(imp_sg, aes(x = Importance_pct, y = Features, fill = GroupDetail)) +
  ...
  labs(x = "Relative Importance", y = NULL,
       title = "Feature Importance (SG Model with 3 Groups - Pathway matrix)") +
  theme_minimal() +
  theme(legend.position = "bottom")

ggsave("importance_epad.png", p_imp,
