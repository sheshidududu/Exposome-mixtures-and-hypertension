# =============================================================================
# E-GO-D model (baseline covariates + clinical indicators + 70 pollutants
# + GO biological feature matrix)
# Stacked generalisation pipeline with elastic-net GO feature screening
# =============================================================================
...
#   go_matrix_raw: pollutant-GO matrix from the ExposomeX platform
...
# go_matrix_raw <- ...   # load GO matrix here
...
# -----------------------------------------------------------------------------
# Feature engineering: GO feature activity (exposure matrix %*% GO matrix)
# -----------------------------------------------------------------------------
...
available_exids <- intersect(existing_exids, rownames(go_matrix_raw))

go_features <- character(0)
if (length(available_exids) > 0) {
  ...
  B <- go_matrix_raw[available_exids, , drop = FALSE] %>% as.matrix()
  B[is.na(B)] <- 0
  T_mat <- P %*% B
  colnames(T_mat) <- colnames(B)
  T_scaled <- apply(T_mat, 2, scale_to_01) %>%
    as_tibble() %>%
    rename_with(~ paste0("GO_", .x))
  eSet$Data <- bind_cols(eSet$Data, T_scaled)
  go_features <- colnames(T_scaled)
} else {
  warning("No pollutants could be matched to the GO matrix; GO features skipped.")
}

# -----------------------------------------------------------------------------
# Elastic-net screening of GO features
...
select_features_elasticnet <- function(features, data, target,
  ...
  if (ncol(df) < 2 || nrow(df) == 0) {
    warning("Insufficient data; no GO features returned.")
    return(character(0))
  }
  ...
  if (length(selected) == 0) {
    warning("No GO feature selected at the current lambda_factor.")
  }
  return(selected)
}

if (length(go_features) > 0) {
  go_features <- select_features_elasticnet(
  ...
  cat("GO feature count after screening:", length(go_features), "\n")
} else {
  cat("No GO features; elastic-net screening skipped.\n")
}
...
groups <- list(
  ...
  Exposome = c(existing_pollutant_cols, go_features)
)
...
imp_sg <- imp_sg %>%
  mutate(GroupDetail = case_when(
    ...
    VarType == "Exposome" & str_detect(Features, "^Exposome_GO_") ~ "GO",
    VarType == "Exposome" & !str_detect(Features, "^Exposome_GO_") ~ "Pollutant",
    TRUE ~ "Other"
  ))
...
p_imp <- ggplot(imp_sg, aes(x = Importance_pct, y = Features, fill = GroupDetail)) +
  ...
  labs(x = "Relative Importance", y = NULL,
       title = "Feature Importance (SG Model with 3 Groups - GO Matrix)") +
  theme_minimal() +
  theme(legend.position = "bottom")

ggsave("importance_egod.png", p_imp,
