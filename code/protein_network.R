# =============================================================================
# E-P-D network: exposure-gene-disease network in Cytoscape
# =============================================================================
...
Nodes_Ge <- bind_rows(nodes_source, nodes_target) %>%
  distinct(id, .keep_all = TRUE) %>%
  mutate(
...
print(paste("NA values in edge table:", sum(is.na(Edges_Ge))))

# -----------------------------------------------------------------------------
# Node degree and size
# -----------------------------------------------------------------------------
degree_source <- Edges_Ge %>% count(source, name = "n_source")
degree_target <- Edges_Ge %>% count(target, name = "n_target")
...
print(paste("Nodes:", nrow(Nodes_Ge)))
print(paste("Edges:", nrow(Edges_Ge)))
print("Node groups:")
print(table(Nodes_Ge$group))
...
createNetworkFromDataFrames(
  nodes = Nodes_Ge,
  edges = Edges_Ge,
  title = "Exposure-Gene-Hypertension Network",
...
setNodeColorMapping(
  mapping.type = 'd',
  table.column = 'group',
  table.column.values = c("Exposure", "Disease", "Gene"),
  colors = c("#E74C3C", "#3498DB", "#F8D582"),
